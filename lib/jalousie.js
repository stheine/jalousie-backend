...
//  logDebug('Start main loop');
  ...
  new Promise(resolve => {
    ...
    if((currentTime.minute() === 0 &&
      ...
      asyncFunctions.weather = weather.getData(globals.config);
    }

// TODO
//    if(currentTime.second() === 30 || temperatureDht === undefined
    if(currentTime.second() === 30 || currentTime.second() === 40 || currentTime.second() === 50 || currentTime.second() === 10 || currentTime.second() === 20 || temperatureDht === undefined
    ) {
...
//       logDebug('get dht22');
// this is the c implementation:      asyncFunctions.dht22 = pigpiod.dht22(globals.pi, 18);
// this is the js implementation      asyncFunctions.dht22 = pigpiod.dht(globals.pi, 18);
      asyncFunctions.dht22 = new Promise(resolve => {
        const dht22Data = pigpiod.dht22Sync(globals.pi, 18);
    ...
    asyncFunctions.windThreshold = wind.getThreshold(globals.pi);

    // Get the sun threshold
    asyncFunctions.sunThreshold = sun.getThreshold(globals.pi);
    ...
    asyncFunctions.kty81Temperature = kty81.getTemperature(globals.pi);
...
//          logDebug('dht22 data received');
          temperatureDht = parseFloat(data.dht22.temperature);
          humidity       = parseFloat(data.dht22.humidity);
      ...
      // Weather
      if(data.weather) {
        handleWeatherData(currentTime, data.weather);
      }
