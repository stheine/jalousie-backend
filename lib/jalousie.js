...
const mainLoop = async function(next) {
  ...
  //        globals.log.debug('get weather');
    ...
    // Publish the data
    await globals.mqttClient.publish('Jalousie/tele/SENSOR', JSON.stringify({
    ...
    if(!weatherData || !nightDownTime) {
      ...
      if(globals.flagWindalarm) {
        ...
        // ohne Verzoegerung.
        if(windLevel < globals.config.wind.up.threshold) {
          if(!timerWind) {
            ...
            globals.log.info(`windLevel(${windLevel}) < ` +
              `wind.up.threshold(${globals.config.wind.up.threshold})`);
            ...
            // Alarm abschalten.
// TODO alarm            globals.mqttClient.publish('Jalousie/cmnd/off', JSON.stringify({}));
...
// TODO alarm          globals.mqttClient.publish('Jalousie/cmnd/up_on', JSON.stringify({}));
        } else { // !windLevel >= config.wind.up.threshold
          if(currentTime.format('HH:mm') >= globals.config.night.windCheck.start &&
            ...
            globals.mqttClient.publish('Jalousie/cmnd/full_up', JSON.stringify({}));
          }
          ...
          if(currentTime.format('HH:mm') === globals.config.night.up) {
            ...
            if(!flagNightAktiv) { // // Prevent triggering multiple times.
              ...
              globals.mqttClient.publish('Jalousie/cmnd/full_down', JSON.stringify({}));
            }
            ...
            if(!globals.flagNight) { // not night
              if(globals.flagSun) {
                ...
                if((sunLevel < globals.config.sun.up.threshold) &&
                   (sunLevel < globals.config.sun.down.temp.threshold ||
                ...
                if((sunLevel >= globals.config.sun.down.threshold) ||
                   (sunLevel >= globals.config.sun.down.temp.threshold &&
                    temperature >= globals.config.sun.down.temp.degree)
                ) {
                  if(!timerSunDown) {
                    ...
                    globals.mqttClient.publish('Jalousie/cmnd/shadow', JSON.stringify({}));
                  }
        ...
        } // !windLevel >= config.wind.up.threshold
      } // !flagWindalarm
    }
...
(async() => {
  ...
  try {
      ...
      try {
        ...
        switch(topic) {
         ...
         case 'Wind/tele/Sensor':
           windLevel = message.level;

           status.update({windLevel});
           // TODO Alarm wenn alarm, dann muessen alle actions geblockt werden.
           break;
    ...
    await globals.mqttClient.subscribe('Sonne/tele/SENSOR');
    await globals.mqttClient.subscribe('Wind/tele/Sensor');
    await globals.mqttClient.subscribe('Wohnzimmer/tele/SENSOR');
