...
const rrdtoolUpdate = async function(rrdUpdates) {
  const {stdout, stderr} = await execAsync(
    `/opt/rrdtool/bin/rrdupdate ` +
      `${rrdFile} ` +
      `--template ${_.keys(rrdUpdates).join(':')} ` +
...
//  globals.log.debug('Start main loop');
  ...
  //    weatherCode:        weatherCode,
    ...
    try {
      await rrdtoolUpdate(rrdUpdates);
    } catch(err) {
      globals.log.error('rrdtoolUpdate', {err: err.message, rrdUpdates});
    }
