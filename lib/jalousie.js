...
const startAction = function(action) {
  actionThread = new Action(action, {
    config: {
      sonneRunterPosition: configSonneRunterPosition,
      sonneRunterWendung:  configSonneRunterWendung
    },
    gpio: {
...
if(pigpiod.callback(pi, GPIO_TASTER_RUNTER, pigpiod.EITHER_EDGE, intrGpioTaster) < 0) {
  logError('Failed to callback(pi, GPIO_TASTER_RUNTER)');

  throw new Error('Failed to callback(pi, GPIO_TASTER_RUNTER)');
}

if(pigpiod.callback(pi, GPIO_TASTER_HOCH, pigpiod.EITHER_EDGE, intrGpioTaster) < 0) {
