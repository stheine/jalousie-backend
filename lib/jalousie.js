...
// *********************************************************************************
// main()
float temperatureOutside  = -999;
int sonnenschwelle;
int windThreshold;
float temperatureKTY     = -999;;
let temperatureDHT;
float humidity  = -999;
double timerWind        = 0;
...
setInterval(() => {
  ...
  promiseAllByKeys(sensorFunctions).then(sensors => {
    ...
    if(sensors.vitoTemperature) {
      status.update({temperatureOutside: sensors.vitoTemperature});
    }
...
//  }
  ...
  //              configNightRunterStunden, configNightRunterMinuten);
            ...
            if((sonnenschwelle >= configSonneRunterSchwelle) ||
              ...
              if(!timerSonneRunter) {
                if(sonnenschwelle >= configSonneRunterSchwelle) {
                  ...
                  logError("sonnenschwelle=%d, temperatureDHT=%.1f",
                    sonnenschwelle, temperatureDHT);
                }
    ...
    status.update({
      ...
      timerSonneHoch:   timerSonneHoch,    // TODO format
      timerSonneRunter: timerSonneRunter,    // TODO format
      timerWind:        timerWind    // TODO format
    });
  });
}, 1000);
