#!/usr/bin/env /home/stheine/.nvm/versions/node/v6.12.0/bin/node
...
//          globals.log.debug('dht22 data received');
        ...
        setTimeout(() => process.nextTick(() => done(null)), 5000);
      } else {
        return process.nextTick(() => done(null));
      }
    },
      ...
      // Get the rain amount
      rain.getRain(globals).then(rainData => {
...
async.waterfall([
    ...
    fsExtra.access(pidFile, fs.constants.W_OK).then(() => {
  ...
  (pidFileExists, done) => {
    ...
    fsExtra.readJson(pidFile).then(pid => {
