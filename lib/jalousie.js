...
const handleWeatherData = function(currentTime, data, sunTimes) {
  ...
  status.update({
    ...
    weatherCloudiness:  data.cloudiness,
...
//        globals.log.debug('dht22 data received');
        ...
        await globals.mqttClient.publish('Wohnzimmer/tele/temperature', String(temperature));
        await globals.mqttClient.publish('Wohnzimmer/tele/humidity', String(humidity));
      } catch(err) {
        dht22Failures++;
...
//        globals.log.debug('sun.getThreshold ', newSunThreshold);
    ...
    if(currentTime.second() === 10 || temperatureOutside === undefined) {
      try {
        ...
        await globals.mqttClient.publish('Aussen/tele/temperature', String(temperatureOutside));
      } catch(err) {
    ...
    try {
      ...
      } while(!stromFile.length && stromRetries);
