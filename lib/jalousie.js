...
const intrGpioTaster = function(intrPi, intrGpio, intrLevel, intrTick) {
  ...
  if(!intrGpioTasterLastTick || !intrGpioTasterTriggerTick) {
    let intrGpioTasterLastTick    = pigpiod.get_current_tick(intrPi);
    let intrGpioTasterTriggerTick = pigpiod.get_current_tick(intrPi);
...
const cleanup = function() {
  return new Promise(resolve => {
    ...
    status.update({process: 'stopped'}).then(() => {
      return resolve();
    });
  });
};
...
setInterval(() => {
  ...
  const sensorFunctions = {
    ...
    sunThreshold:     sun.getThreshold(pi),
    windThreshold:    wind.getThreshold(pi),
    vitoTemperature:  vito.getTemperature()
  };

  // read room temperature and humidity from DHT22 sensor
  // only once per minute (at seconds === 30)
  ...
  promiseAllByKeys(sensorFunctions).then(sensors => {
    ...
    if(sensors.windThreshold) {
      ...
      windThreshold = sensors.windThreshold;
    ...
    // Sun
    if(sensors.sunThreshold) {
      sunThreshold = sensors.sunThreshold;
    ...
    if(sensors.kty81) {
      ...
      status.update({
        temperaturKTY: sensors.kty81Temperature
      });
    }
    ...
    if(sensors.dht22Data) {
      if(sensors.dht22Data.status === pigpiod.dht.DHT_GOOD) {
        ...
        status.update({
          ...
          dht22Timestamp:  sensors.dht22.timestamp.format('HH:mm:ss')
        });
      } else {
        logInfo(
