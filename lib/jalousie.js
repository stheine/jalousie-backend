...
// TODO die status flags (Wind/ Sonne/ Auto) muss ich auch in files schreiben
...
const jalousieAction = function(action) {
  switch(action) {
    case JALOUSIE_GANZHOCH:
      ...
      gpio_write(pi, GPIO_JALOUSIE_HOCH, JALOUSIE_AN);
      time_sleep(3); // 3 Sekunden
      gpio_write(pi, GPIO_JALOUSIE_HOCH, JALOUSIE_AUS);
      break;

    case JALOUSIE_GANZRUNTER:
      logMsg("jalousieAction GanzRunter: JALOUSIE_RUNTER, AN, 3sek, AUS");
      gpio_write(pi, GPIO_JALOUSIE_RUNTER, JALOUSIE_AN);
      time_sleep(3); // 3 Sekunden
      gpio_write(pi, GPIO_JALOUSIE_RUNTER, JALOUSIE_AUS);
      break;
    ...
    case JALOUSIE_SCHATTEN:
      ...
      gpio_write(pi, GPIO_JALOUSIE_RUNTER, JALOUSIE_AN); // Stop
...
// TODO    }
    ...
    if(level &&
      ...
      realLevel = gpio_read(pi, userGpio);
      if(realLevel != level) {
...
//      stateTasterHoch = gpio_read(pi, GPIO_TASTER_HOCH);
...
//      stateTasterRunter = gpio_read(pi, GPIO_TASTER_RUNTER);
    }


    // Jetzt kann ich die Tastendruck weitergeben.

    logDebug("intrGpioTaster(%d, %d) realLevel=%d", userGpio, level, gpio_read(pi, userGpio));
    logInfo("intrGpioTaster(%d, %d) realLevel=%d", userGpio, level, gpio_read(pi, userGpio));
    logInfo("intrGpioTaster(%d, %d)", userGpio, level);

    logInfo("Taster %s, %s %d", tasterString, levelString, tickSinceLast);

    if(flagWindalarm) {
      logMsg("flagWindalarm unterdrueckt gpio_write(pi, d, %d)", outDirection,
        outLevel);
    } else {
      gpio_write(pi, outDirection, outLevel);
    }
  } // Stop
...
// input, pull-up
set_mode(pi, GPIO_TASTER_RUNTER, PI_INPUT);
set_pull_up_down(pi, GPIO_TASTER_RUNTER, PI_PUD_UP);
set_glitch_filter(pi, GPIO_TASTER_RUNTER, 50);

set_mode(pi, GPIO_TASTER_HOCH, PI_INPUT);
set_pull_up_down(pi, GPIO_TASTER_HOCH, PI_PUD_UP);
set_glitch_filter(pi, GPIO_TASTER_HOCH, 50);

set_mode(pi, GPIO_WIND, PI_INPUT);
set_pull_up_down(pi, GPIO_WIND, PI_PUD_UP);

// output, init 0 -> Transistor open -> Jalousie pull-up remains on 5V.
set_mode(pi, GPIO_JALOUSIE_HOCH, PI_OUTPUT);
gpio_write(pi, GPIO_JALOUSIE_HOCH, JALOUSIE_AUS);
logMsg("Init: JALOUSIE_HOCH, AUS");

set_mode(pi, GPIO_JALOUSIE_RUNTER, PI_OUTPUT);
gpio_write(pi, GPIO_JALOUSIE_RUNTER, JALOUSIE_AUS);
...
if(callback(pi, GPIO_TASTER_RUNTER, EITHER_EDGE, &intrGpioTaster) < 0) {
  logError("Failed to callback(pi, GPIO_TASTER_RUNTER)");
  return(1);
}

if(callback(pi, GPIO_TASTER_HOCH, EITHER_EDGE, &intrGpioTaster) < 0) {
...
if(callback(pi, GPIO_WIND, FALLING_EDGE, &intrGpioWind) < 0) {
...
// darauf agiert.
setInterval(() => {
  loopStartTick = get_current_tick(pi);
  ...
  if(stateTasterRunter === 0 &&
     gpio_read(pi, GPIO_TASTER_RUNTER) === 1
  ) {
  ...
  if(stateTasterHoch === 0 &&
     gpio_read(pi, GPIO_TASTER_HOCH) === 1
  ) {
...
//         gpio_read(pi, GPIO_JALOUSIE_RUNTER) === JALOUSIE_AN
//      ) {
//        logMsg("Haengenden Ausgang JALOUSIE_RUNTER gefixt");
//        gpio_write(pi, GPIO_TASTER_RUNTER, 0);
//        stateTasterRunter = 0;
//      }
//    }
  ...
  if(flagWindalarm) {
    ...
    if(windschwelle < configWindHochSchwelle) {
      if(!timerWind) {
        ...
        gpio_write(pi, GPIO_JALOUSIE_HOCH, JALOUSIE_AUS); // Alarm abschalten.
        if(flagSchaltzeit) {
    ...
    if(windschwelle >= configWindHochSchwelle) {
      ...
      gpio_write(pi, GPIO_JALOUSIE_HOCH, JALOUSIE_AN); // Alarm. Dauersignal HOCH.
