...
const intrGpioTaster = function(intrPi, intrGpio, intrLevel, intrTick) {
  ...
  if(!intrGpioTasterLastTick || !intrGpioTasterTriggerTick) {
    intrGpioTasterLastTick    = pigpiod.get_current_tick(intrPi);
    intrGpioTasterTriggerTick = pigpiod.get_current_tick(intrPi);
...
//    }
    ...
    rrdFields.forEach(key => {
      const value = eval(key); // eigentlich waere es besser, das anders zu implementieren.
