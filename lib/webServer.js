...
//      globals.log.debug(`no authentication needed for ${req.path}`);
  ...
  appPort.get('/login', passportPort.authenticate('digest', {session: false}), (req, res) => {
...
//        globals.log.info(`Redirecting to original url ${url}`);
  ...
  appProxy.get('/login', passportProxy.authenticate('reverseproxy', { session: false }), (req, res) => {
    // Authentication successful. Set user into session
