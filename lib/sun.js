...
const sunRingbufferDates  = new Ringbuffer(SUN_RING_BUFFER_LENGTH);
const sunRingbufferValues = new Ringbuffer(SUN_RING_BUFFER_LENGTH);
let   sunLastThreshold;

const getThreshold = async function(/* params */) {
  let   sunThreshold;
...
//  params.log.debug(`sun a2dValue=${a2dValue}`, a2dValueRaw);

  sunRingbufferDates.enq(now);
  sunRingbufferValues.enq(a2dValue);

  // Remove outdated values from the ringbuffer.
  while(
    sunRingbufferDates.size() &&
    (now - sunRingbufferDates.peek()) > (SUN_RING_BUFFER_LENGTH + 15) * 1000
  ) {
//      params.log.debug('Removing outdated value from sun ringbuffer');
    sunRingbufferValues.deq();
    sunRingbufferDates.deq();
  }

//  if(sunRingbufferValues.size() < SUN_RING_BUFFER_LENGTH) {
//    params.log.debug(`SUN_RING_BUFFER size=${sunRingbufferValues.size()}`);
//  }

  const averageA2dValue = sunRingbufferValues.avg();

//  params.log.debug(`${sunRingbufferValues.dump()} => avg()=${averageA2dValue}`);
...
//  params.log.debug(`Sun: ${sunThreshold} averageA2dValue=${averageA2dValue}`);

  if(sunLastThreshold !== sunThreshold) {
//    params.log.debug(`SunThreshold: ${sunThreshold} / ${averageA2dValue}`);
    sunLastThreshold = sunThreshold;
  }

  return {
    threshold: sunThreshold,
    timestamp: moment(),
  };
};
