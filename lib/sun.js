...
const getThreshold = function(globals) {
  return new Promise(resolve => {
    ...
    const a2dValue = pigpiod.mcp3204(globals.pi, globals.spi, MCP3204_CHANNEL_SUN);
    ...
    while(sunRingbufferTicks.size() &&
          (nowTicks - sunRingbufferTicks.peek()) >
            2 * (SUN_RING_BUFFER_LENGTH + 5) * 1000000
    ) {
      globals.log.debug('Removing outdated value from sun ringbuffer');
      sunRingbufferValues.deq();
      sunRingbufferTicks.deq();
    }

    if(sunRingbufferValues.size() < SUN_RING_BUFFER_LENGTH) {
      globals.log.debug(`SUN_RING_BUFFER size=${sunRingbufferValues.size()}`);
    }

    const averageA2dValue = sunRingbufferValues.avg();

//    globals.log.debug(`${sunRingbufferValues.dump()} => avg()=${averageA2dValue}`);
...
//    globals.log.debug(`Sun: ${sunThreshold} averageA2dValue=${averageA2dValue}`);

    if(sunLastThreshold !== sunThreshold) {
//      globals.log.debug(`SunThreshold: ${sunThreshold} / ${averageA2dValue}`);
