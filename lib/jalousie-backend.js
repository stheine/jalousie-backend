...
const mainLoop = async function({config, mqttClient}) {
  try {
    ...
    if(!nightDownTime) {
      ...
      if(currentTime.format('HH:mm') >= config.night.windCheck.start &&
        currentTime.format('HH:mm') <= config.night.windCheck.end &&
        forecastMaxWind > config.night.windCheck.limit
      ) {
        logger.info(`forecastMaxWind(${forecastMaxWind}) > limit(${config.night.windCheck.limit})`);
...
(async() => {
  ...
  try {
    ...
    const weatherInterval = setInterval(async() => {
      nightDownTime = await weather.getNightDownTime({
