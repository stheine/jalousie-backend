...
const getTemperature = function(globals) {
  return new Promise(resolve => {
    let   temperature;
    const nowTicks = pigpiod.get_current_tick(globals.pi);
    const a2dValue = pigpiod.mcp3204(globals.pi, globals.spi, MCP3204_CHANNEL_KTY81);

    // Remove outdated values from the ringbuffer.
    while(tempRingbufferTicks.size() &&
          (nowTicks - tempRingbufferTicks.peek()) >
            2 * (TEMP_RING_BUFFER_LENGTH + 5) * 1000000
    ) {
      globals.log.debug('Removing outdated value from temp ringbuffer');
      tempRingbufferValues.deq();
      tempRingbufferTicks.deq();
    }

    if(tempRingbufferValues.size() < TEMP_RING_BUFFER_LENGTH) {
      globals.log.debug(`TEMP_RING_BUFFER size=${tempRingbufferValues.size()}`);
    }

    const resistance = a2dValue / (4095 - a2dValue) * kty81DeviderResistor;

    if(resistance < _.first(kty81ResistorValues)) {
      globals.log.error('KTY81 Temperature sensor shortcut');
      temperature = -999;
    } else if(resistance > _.last(kty81ResistorValues)) {
      globals.log.error('KTY81 Temperature sensor missing');
      temperature =  999;
    } else {
...
//          globals.log.debug(
//            `${resistorPrev}Ω  ${resistance.toFixed(0)}Ω  ` +
//              `${resistorCurr}Ω\n` +
//            `[${i-1}]    ${(pct * 100).toFixed(0)}%   [${i}]\n` +
//            `${tempPrev.toFixed(1)}°C ${temperature.toFixed(1)}°C ` +
//              `${tempCurr.toFixed(1)}°C`);
...
//    globals.log.debug(`KTY81: a2dValue=${a2dValue.toFixed(0)} ` +
...
//    globals.log.debug(`${tempRingbufferValues.dump()} ` +
//      `avg()=${tempRingbufferValues.avg().toFixed(1)}`);
