  ...
  return new Promise((resolve, reject) => {
    ...
    while(tempRingbufferTicks.size() &&
          (nowTicks - tempRingbufferTicks.peek()) >
            (TEMP_RING_BUFFER_LENGTH + 1) * 1000000
    ) {
    ...
    if(resistance < _.first(kty81ResistorValues)) {
      logWarn('KTY81 Temperature sensor shortcut');
      temperature = -999;
    } else if(resistance > _.last(kty81ResistorValues)) {
      logWarn('KTY81 Temperature sensor missing');
      temperature =  999;
    } else {
...
//            `${tempPrev.toFixed(1)}°C ${temperature.toFixed(1)}°C ${tempCurr.toFixed(1)}°C`);
    ...
    logDebug(`KTY81: a2dValue=${a2dValue.toFixed(0)} resistance=${resistance.toFixed(0)} temperature=${temperature.toFixed(1)}°C`);
    ...
    return resolve(averageTemperature);
  });
};
