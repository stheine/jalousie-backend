...
const getTemperature = function(pi, spi) {
  return new Promise(resolve => {
    let   temperature;
    const nowTicks = pigpiod.get_current_tick(pi);
    const a2dValue = pigpiod.mcp3204(pi, spi, MCP3204_CHANNEL_KTY81);
    ...
    if(tempRingbufferValues.size() < TEMP_RING_BUFFER_LENGTH) {
      logDebug(`TEMP_RING_BUFFER size=${tempRingbufferValues.size()}`);
    }
...
//    logDebug(`${tempRingbufferValues.dump()} ` +
