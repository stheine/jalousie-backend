...
//    timeout:    xxx milliseconds  // interrupt only
  ...
  gpioWind.on('interrupt', level => {
    log.debug(`triggerWind interrupt raw(${level})`);
  });

  gpioWind.on('alert', level /* , tick */ => {
...
// timeout interrupt only    if(level === Gpio.TIMEOUT) {
...
//    log.debug(`triggerWind()`, sinceLast);
