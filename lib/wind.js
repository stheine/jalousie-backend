...
const getWindThreshold = function(pi) {
  ...
  //  trace("windThreshold=%d", windThreshold);

  //  trace("Wind:  %d %d/%3.1f (%05.2fHz)", windThreshold, windCounterCurrent,
  //    windSecondsSinceLast, windHertz);

// TODO    writeStatus("windThreshold", FMT_INT_02, (void *)&windThreshold);
// TODO    writeStatus("windHertz",    FMT_FLOAT_05_2, (void *)&windHertz);

    if(windLastThreshold !== windThreshold &&
       windThreshold > 1
    ) {
  //    logInfo("windThreshold: %02d / #%d/%3.1fs / %05.2fHz",
  //      windThreshold, windCounterCurrent, windSecondsSinceLast, windHertz);
      windLastThreshold = windThreshold;
    }

    return resolve(windThreshold);
  });
}
