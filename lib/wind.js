...
const trigger = function(pi, gpio, level, tick) {
  ...
  if(tickSinceLast < 10000) {
    ...
    log.debug('Supressing wind phantom interrupt');
...
const initialize = function(globals) {
  log = globals.log;

  if(pigpiod.callback(globals.pi, GPIO_WIND, pigpiod.FALLING_EDGE, trigger) < 0) {
    globals.log.error('Failed to callback(pi, GPIO_WIND)');
...
//    globals.log.debug(`windCounter=${windCounter}`);
    ...
    if(!windLastTicks) {
      globals.log.info('Initialize WindCounter');

      windLastTicks       = pigpiod.get_current_tick(globals.pi);
      windTriggerLastTick = pigpiod.get_current_tick(globals.pi);
