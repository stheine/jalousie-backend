        ...
        `woeid in ` +
          `( ` +
            ...
            `from ` +
              `geo.places(1) ` +
            `where ` +
              `text='${config.location.city}, ${config.location.region}, ` +
...
const getNightDownTime = function(weatherData, currentTime, lastDownTime) {
  let offsetMinutes;
  ...
  switch(parseInt(weatherData.code, 10)) {
    ...
    default:
      logError(`Unhandled weatherData.code=${weatherData.code}`);
      offsetMinutes = 0;
      break;
  }

  let newDownTime = weatherData.sunset.add(offsetMinutes, 'minutes');

  if(!lastDownTime) {
    // Die night down time had not been calculated yet.
    logInfo(`Night down time initial calculation:\n` +
    ...
    if(newDownTime.isBefore(currentTime) &&
      ...
      logInfo(`New night down time has already passed -> TRIGGER NOW\n` +
        `  sunset: ${weatherData.sunset.format('HH:mm')}\n` +
        `  code: ${weatherData.code} (${weatherData.text}), ` +
            `offset=${offsetMinutes}\n` +
        `  downTime: ${newDownTime.format('HH:mm')}`);
      ...
      logInfo(`Night down time update:\n` +
        `  sunset: ${weatherData.sunset.format('HH:mm')}\n` +
        `  code: ${weatherData.code} (${weatherData.text}), ` +
            `offset=${offsetMinutes}\n` +
        `  downTime: ${newDownTime.format('HH:mm')}`);
