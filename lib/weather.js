...
const getData = async function(globals) {
  ...
  const weather = await request({
    json:     true,
    url:      `http://${host}${uri}?q=${encodeURI(query)}&format=json`,
    timeout:  2000, // 2 seconds
  });

  // Confirm validity of the result
  check.assert.not.zero(_.get(weather, ['query', 'count'], 0), 'No weather data');
  check.assert(check.all([
    check.assigned(_.get(weather, ['query', 'results', 'channel', 'location', 'city'])),
    check.assigned(_.get(weather, ['query', 'results', 'channel', 'location', 'country'])),
    check.assigned(_.get(weather, ['query', 'results', 'channel', 'location', 'region'])),
    check.assigned(_.get(weather, ['query', 'results', 'channel', 'astronomy', 'sunrise'])),
    check.assigned(_.get(weather, ['query', 'results', 'channel', 'astronomy', 'sunset'])),
    check.assigned(_.get(weather, ['query', 'results', 'channel', 'item', 'condition', 'code'])),
    check.assigned(_.get(weather, ['query', 'results', 'channel', 'item', 'condition', 'text'])),
  ], `Incomplete weather data in ${JSON.stringify(weather, null, '  ')}`));
  check.assert.equal(weather.query.results.channel.location.city.trim(),
    globals.config.location.city,
    `Weather data city mismatch ` +
    `'${weather.query.results.channel.location.city}!==${globals.config.location.city}'`);
  check.assert.equal(weather.query.results.channel.location.country.trim(),
    globals.config.location.country,
    `Weather data country mismatch ` +
    `'${weather.query.results.channel.location.country}!==${globals.config.location.country}'`);
  check.assert.equal(weather.query.results.channel.location.region.trim(),
    globals.config.location.region,
    `Weather data region mismatch ` +
    `'${weather.query.results.channel.location.region}!==${globals.config.location.region}'`);
  ...
  return {
    sunrise: moment(weather.query.results.channel.astronomy.sunrise, 'h:m a'),
    sunset:  moment(weather.query.results.channel.astronomy.sunset,  'h:m a'),
...
const getNightDownTime = function(globals, weatherData, sunTimes, currentTime, lastDownTime) {
  ...
  switch(parseInt(weatherData.code, 10)) {
    ...
    case 40: // scattered showers / heavy rain
      offsetMinutes = -60;
      break;
