...
const getData = async function({openWeatherLocation, suncalcLocation}) {
  ...
  try {
    const resultRaw = await await axios({
      method: 'get',
      url: `https://${host}${base}/onecall?` +
    ...
    check.assert.equal(resultRaw.status, 200, `Unexpected status=${resultRaw.status}`);

    const {data} = resultRaw;
...
export default {
  async get({openWeatherLocation, suncalcLocation}) {
